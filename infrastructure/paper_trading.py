...
class PaperTradingEngine:
    ...
    def fill_order(self, order: PaperOrder, fill_price: float, timestamp: Optional[datetime] = None):
        """Simulate order fill."""
        order.status = "filled"
    ...
    def get_unrealized_pnl(self, prices: Dict[str, float]) -> Dict[str, float]:
        ...
        for market_id, position in self.positions.items():
            if position != 0 and market_id in prices:
                ...
                if market_trades:
                    avg_entry = sum(t["size"] * t["price"] for t in market_trades) / sum(t["size"] for t in market_trades)
    ...
    def get_metrics(self) -> dict:
        ...
        equity_series = pd.Series([e["total_equity"] for e in self.equity_history])
