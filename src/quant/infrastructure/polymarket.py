...
class PolymarketClient:
    ...
    def get_prices(
        ...
        response = self.session.get(f"{self.base_url}/api/price/{market_id}", params=params)
        response.raise_for_status()
        return response.json()
    ...
    def place_order(
        ...
        response = self.session.post(f"{self.base_url}/api/order", json=payload)
        response.raise_for_status()
        return response.json()
    ...
    def cancel_order(self, order_id: str) -> dict:
        ...
        Returns:
            Cancel response
        """
        response = self.session.delete(f"{self.base_url}/api/order/{order_id}")
        response.raise_for_status()
        return response.json()
    ...
    def get_unrealized_pnl(self, prices: Dict[str, float]) -> Dict[str, float]:
        ...
        for market_id, position in self.positions.items():
            if position != 0 and market_id in prices:
                ...
                market_trades = [t for t in self.trades if t["marketId"] == market_id]
                if market_trades:
                    avg_entry = sum(t["size"] * t["fillPrice"] for t in market_trades) / sum(
