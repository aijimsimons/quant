...
class AlertRules:
    ...
    @staticmethod
    def check_drawdown(
        alert_manager: AlertManager, equity: float, peak: float, threshold: float = 0.10
    ):
        ...
        if peak > 0:
            ...
            if drawdown >= threshold:
                alert_manager.create(
                    ...
                    message=f"Drawdown alert: {drawdown * 100:.2f}%",
    ...
    def check_high_volatility(
        alert_manager: AlertManager, volatility: float, threshold: float = 0.05
    ):
