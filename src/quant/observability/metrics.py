...
class StrategyMetrics:
    ...
    def calculate(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        metrics["total_return"] = (
            (equity.iloc[-1] - self.capital) / self.capital if len(equity) > 1 else 0
        )
        ...
        if len(returns) > 0:
            metrics["sharpe_ratio"] = (
                (returns.mean() / returns.std()) * (252 * 24) ** 0.5 if returns.std() > 0 else 0
            )
        else:
        ...
        if len(returns) > 0:
            metrics["sortino_ratio"] = (
                (returns.mean() / returns[returns < 0].std()) * (252 * 24) ** 0.5
    ...
    def calculate(self, strategy_results: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        if len(total_equity) > 1:
            portfolio_metrics["total_return"] = (
                total_equity.iloc[-1] - self.capital
            ) / self.capital
            ...
            if len(returns) > 0 and returns.std() > 0:
                portfolio_metrics["sharpe_ratio"] = (returns.mean() / returns.std()) * (
                    252 * 24
                ) ** 0.5
