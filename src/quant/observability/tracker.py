...
class StrategyTracker:
    ...
    def record(
        self, strategy_name: str, metrics: Dict[str, Any], timestamp: Optional[datetime] = None
    ):
        """Record strategy metrics."""
        if timestamp is None:
            timestamp = datetime.now()

        record = {"timestamp": timestamp, "strategy": strategy_name, **metrics}
    ...
    def get_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for strategy_name, records in self.history.items():
            if records:
                ...
                if len(records) > 1:
                    first = records[0]
                    summary[strategy_name]["total_change"] = latest.get(
                        "total_return", 0
                    ) - first.get("total_return", 0)
