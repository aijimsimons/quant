...
def mean_reversion_polymarket(
    ...
    max_holding_period: int = 20,  # 20 five-minute bars = 100 minutes
    min_zscore: float = -0.8,
    max_zscore: float = 0.8,
    slippage_pct: float = 0.001,  # 0.1% bid-ask spread
    commission: float = 0.0,  # Polymarket has no commission
    verbose: bool = False,
    ...
    # Generate signals - note: we use shift(1) to avoid look-ahead bias
    df = df.with_columns([
    ...
    for i in range(window + 1, n):  # +1 because of shift(1)
        ...
        if position == 0:
            ...
            if current_signal == 1:
                ...
                entry_time = i
                if verbose:
                    print(f"   Entry LONG at i={i}, price={current_price:.2f}, entry_price={entry_price:.2f}, position={position}")
                ...
                entry_time = i
                if verbose:
                    print(f"   Entry SHORT at i={i}, price={current_price:.2f}, entry_price={entry_price:.2f}, position={position}")
...
def calculate_metrics(df: Union[pl.DataFrame, pd.DataFrame], capital: float = 10000.0) -> dict:
    ...
    if len(returns) == 0 or returns.std() == 0:
        ...
        # Annualize for 5-minute data: 252 days * 24 hours * 12 five-min periods = 72576 periods
